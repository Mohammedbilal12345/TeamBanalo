...
const Profile: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: user?.email || profile?.email || '', // Use user.email as primary source
  ...
  const handleSave = async () => {
    try {
      ...
      try {
        const response = await fetch(`https://teambanalo.onrender.com/${user.id}`);
        if (!response.ok) {
          toast({
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      onChange={handleInputChange}
                      className="input-dark pl-11 bg-gray-700/50 cursor-not-allowed"
                      readOnly
                      disabled
